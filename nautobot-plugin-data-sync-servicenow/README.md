# Nautobot ServiceNow Data Synchronization
...
```shell
pip install nautobot-data-sync-servicenow
```

> The plugin is compatible with Nautobot 1.0.0 and higher

To ensure Nautobot ServiceNow Data Synchronization is automatically re-installed during future upgrades, create a file named `local_requirements.txt` (if not already existing) in the Nautobot root directory (alongside `requirements.txt`) and list the `nautobot-data-sync-servicenow` package:

```no-highlight
# echo nautobot-data-sync-servicenow >> local_requirements.txt
```
...
* `project_name`: the default docker compose project name (default: nautobot-data-sync-servicenow)
...
Each command can be executed with `invoke <command>`. Environment variables `INVOKE_NAUTOBOT-DATA-SYNC-SERVICENOW_PYTHON_VER` and `INVOKE_NAUTOBOT-DATA-SYNC-SERVICENOW_NAUTOBOT_VER` may be specified to override the default versions. Each command also has its own help `invoke <command> --help`
