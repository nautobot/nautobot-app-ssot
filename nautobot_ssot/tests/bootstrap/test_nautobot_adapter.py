        ...
        if isinstance(item, dict):
            for item_key in list(item.keys()):
                if item_key in ["system_of_record", "model_flags", "uuid"]:
                    ...
                    item[item_key] = None
                elif item_key in keys_to_normalize and (
                    item.get(item_key) is None or item.get(item_key) == ""
                ):
                    item[item_key] = None
                ...
                if (
                    item_key == "content_types"
                    or item_key == "provided_contents"
                    and isinstance(item[item_key], list)
                ):
