...
class TestNautobotUtils(TransactionTestCase):  # pylint: disable=too-many-instance-attributes
    ...
    def setUp(self):
        ...
        self.adapter.objects_to_create = {
            "platforms": [],
            "vlans": [],
            "tagged_vlans": [],
        }
    ...
    def test_determine_vc_position(self):
        ...
        sw1_pos = determine_vc_position(
            vc_map=vc_map,
            virtual_chassis="switch_vc_example",
            device_name="switch_vc_example - Switch 1",
        )
        self.assertEqual(sw1_pos, 2)
        sw2_pos = determine_vc_position(
            vc_map=vc_map,
            virtual_chassis="switch_vc_example",
            device_name="switch_vc_example - Switch 2",
        )
        self.assertEqual(sw2_pos, 3)
        node3_pos = determine_vc_position(
            vc_map=vc_map,
            virtual_chassis="node_vc_example",
            device_name="node_vc_example - node2",
        )
        self.assertEqual(node3_pos, 4)
        fw_pos = determine_vc_position(
            vc_map=vc_map,
            virtual_chassis="firewall_pair_example",
            device_name="firewall - FTX123456AB",
        )
    ...
    def test_update_custom_fields_remove_cf(self):
        ...
        test_location = Location.objects.create(
            name="Test",
            location_type=LocationType.objects.get_or_create(name="Region")[0],
            status=self.status_active,
        )
        _cf_dict = {
            "key": "department",
        ...
        self.assertFalse(
            test_location.custom_field_data.get("Department"),
            "department should not exist in the dictionary",
        )
