...
def process_ext_attrs(adapter, obj: object, extattrs: dict):  # pylint: disable=too-many-branches
    ...
    Args:
        adapter (object): DiffSync Adapter
            ...
            if attr.lower() == "vrf":
                if isinstance(attr_value, list):
                    for vrf in attr_value:
                        try:
                            obj.vrfs.add(adapter.vrf_map[vrf])
                        except KeyError as err:
                            adapter.job.logger.warning(
                    ...
                    try:
                        obj.vrfs.add(adapter.vrf_map[attr_value])
                    except KeyError as err:
                        adapter.job.logger.warning(
...
def _create_ip_address_common(adapter: object, ids: dict, attrs: dict) -> IPAddress:
    """Creates common IP Address atrributes.

    Args:
        adapter (object): diffsync adapter instance
    ...
    if attrs.get("ext_attrs"):
        process_ext_attrs(adapter=adapter, obj=_ip, extattrs=attrs["ext_attrs"])
...
class NautobotNetwork(Network):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        adapter.prefix_map[(ids["namespace"], ids["network"])] = _prefix.id
    ...
    def update(self, attrs):  # pylint: disable=too-many-branches
        ...
        if self.adapter.job.debug:
            self.adapter.job.logger.debug(f"Attempting to update Prefix {_pf.prefix} with {attrs}.")
        if "description" in attrs:
            _pf.description = attrs["description"]
        if "network_type" in attrs:
            _pf.type = attrs["network_type"]
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=self.adapter, obj=_pf, extattrs=attrs["ext_attrs"])
        prefix_ranges = attrs.get("ranges")
        if prefix_ranges:
            _pf.cf["dhcp_ranges"] = ",".join(prefix_ranges)
        # Only attempt associating to VLANs if they were actually loaded
        if "vlans" in attrs and self.adapter.vlan_map:  # pylint: disable=too-many-nested-blocks
            ...
            if len(current_vlans) < len(attrs["vlans"]):
                ...
                for vlan in current_vlans:
                    if vlan.vid not in attrs["vlans"]:
                        ...
                        if self.adapter.job.debug:
                            self.adapter.job.logger.debug(f"Removing VLAN {vlan.vid} from {_pf.prefix}.")
                        del_vlan.delete()
        _pf.validated_save()
...
class NautobotIPAddress(IPAddress):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        if adapter.config.fixed_address_type == FixedAddressTypeChoices.DONT_CREATE_RECORD:
            adapter.job.logger.warning(
        ...
        if adapter.job.debug:
            adapter.job.logger.debug(f"Creating IP Address {addr_w_pfxl}")
        _ip = _create_ip_address_common(adapter, ids, attrs)
        ...
        try:
            _ip.validated_save()
            adapter.ipaddr_map[(f"{addr_w_pfxl}", ids["namespace"])] = _ip.id
            return super().create(ids=ids, adapter=adapter, attrs=attrs)
        except ValidationError as err:
            adapter.job.logger.warning(f"Error with validating IP Address {addr_w_pfxl}-{ids['namespace']}. {err}")
            return None
    ...
    def update(self, attrs):  # pylint: disable=too-many-branches
        ...
        if attrs.get("description") == "" and FixedAddressTypeChoices.DONT_CREATE_RECORD:
            ...
            try:
                ...
                self.adapter.job.logger.warning(f"Error with updating IP Address {self.address}. {err}")
                return None
        ...
        if attrs.get("status"):
            try:
                status = self.adapter.status_map[attrs["status"]]
            except KeyError:
                status = self.adapter.config.default_status.pk
        ...
        if attrs.get("description"):
            _ipaddr.description = attrs["description"]
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=self.adapter, obj=_ipaddr, extattrs=attrs["ext_attrs"])
        if "mac_address" in attrs:
    ...
    def delete(self):
        ...
        if NautobotDeletableModelChoices.IP_ADDRESS not in self.adapter.config.nautobot_deletable_models:
            return super().delete()

        _ipaddr = OrmIPAddress.objects.get(id=self.pk)
        del self.adapter.ipaddr_map[_get_ip_address_ds_key(self)]
        _ipaddr.delete()
        return super().delete()
...
class NautobotVlanGroup(VlanView):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        if attrs.get("ext_attrs"):
            process_ext_attrs(adapter=adapter, obj=_vg, extattrs=attrs["ext_attrs"])
        _vg.validated_save()
        adapter.vlangroup_map[ids["name"]] = _vg.id
        return super().create(ids=ids, adapter=adapter, attrs=attrs)
    ...
    def update(self, attrs):
        ...
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=self.adapter, obj=_vg, extattrs=attrs["ext_attrs"])
        return super().update(attrs)
...
class NautobotVlan(Vlan):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        _vlan = OrmVlan(
            vid=ids["vid"],
            name=ids["name"],
            status_id=adapter.status_map[cls.get_vlan_status(attrs["status"])],
            vlan_group_id=adapter.vlangroup_map[ids["vlangroup"]],
            description=attrs["description"],
        )
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=adapter, obj=_vlan, extattrs=attrs["ext_attrs"])
        try:
            _vlan.tags.add(create_tag_sync_from_infoblox())
            _vlan.validated_save()
            if ids["vlangroup"] not in adapter.vlan_map:
                adapter.vlan_map[ids["vlangroup"]] = {}
            adapter.vlan_map[ids["vlangroup"]][_vlan.vid] = _vlan.id
            return super().create(ids=ids, adapter=adapter, attrs=attrs)
        except ValidationError as err:
            adapter.job.logger.warning(f"Unable to create VLAN {ids['name']} {ids['vid']}. {err}")
            return None
    ...
    def update(self, attrs):
        ...
        if attrs.get("status"):
            _vlan.status_id = self.adapter.status_map[self.get_vlan_status(attrs["status"])]
        if attrs.get("description"):
            _vlan.description = attrs["description"]
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=self.adapter, obj=_vlan, extattrs=attrs["ext_attrs"])
        ...
        try:
            ...
            self.adapter.job.logger.warning(f"Unable to update VLAN {_vlan.name} {_vlan.vid}. {err}")
            return None
        return super().update(attrs)
...
class NautobotNamespace(Namespace):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        if attrs.get("ext_attrs"):
            process_ext_attrs(adapter=adapter, obj=_ns, extattrs=attrs["ext_attrs"])
        try:
            _ns.tags.add(create_tag_sync_from_infoblox())
            _ns.validated_save()
            adapter.namespace_map[ids["name"]] = _ns.id
            return super().create(ids=ids, adapter=adapter, attrs=attrs)
        except ValidationError as err:
            adapter.job.logger.warning(f"Unable to create Namespace {_ns.name}. {err}")
            return None
    ...
    def update(self, attrs):
        ...
        if "ext_attrs" in attrs:
            process_ext_attrs(adapter=self.adapter, obj=_ns, extattrs=attrs["ext_attrs"])
        try:
            _ns.validated_save()
            return super().update(attrs)
        except ValidationError as err:
            self.adapter.job.logger.warning(f"Unable to update Namespace {_ns.name}. {err}")
            return None
...
class NautobotDnsARecord(DnsARecord):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        if ip_pk:
            ...
            if adapter.job.debug:
                adapter.job.logger.debug(f"Creating IP Address from A record data: {addr_w_pfxl}-{ids['namespace']}.")
            try:
                _ipaddr = _create_ip_address_common(adapter, ids, attrs)
                _ipaddr.dns_name = attrs.get("dns_name") or ""
                _ipaddr.custom_field_data.update({"dns_a_record_comment": attrs.get("description") or ""})
                _ipaddr.validated_save()
                adapter.ipaddr_map[(addr_w_pfxl, ids["namespace"])] = _ipaddr.id
            except ValidationError as err:
                adapter.job.logger.warning(
    ...
    def delete(self):
        ...
        if NautobotDeletableModelChoices.DNS_A_RECORD not in self.adapter.config.nautobot_deletable_models:
            return super().delete()
        ...
        if _get_ip_address_ds_key(self) not in self.adapter.ipaddr_map:
            return super().delete()
...
class NautobotDnsHostRecord(DnsHostRecord):
    ...
    def delete(self):
        ...
        if NautobotDeletableModelChoices.DNS_HOST_RECORD not in self.adapter.config.nautobot_deletable_models:
            return super().delete()
...
class NautobotDnsPTRRecord(DnsPTRRecord):
    ...
    def delete(self):
        ...
        if NautobotDeletableModelChoices.DNS_PTR_RECORD not in self.adapter.config.nautobot_deletable_models:
            return super().delete()
