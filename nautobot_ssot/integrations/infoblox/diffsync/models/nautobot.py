...
class NautobotNetwork(Network):
    ...
    #     self.adapter.job.logger.warning(f"Prefix {self.network} will be deleted.")
...
class NautobotIPAddress(IPAddress):
    ...
    def create(cls, adapter, ids, attrs):
        ...
        _ip = OrmIPAddress(
            ...
            parent_id=adapter.prefix_map[ids["prefix"]],
        )
        if attrs.get("ext_attrs"):
        ...
        try:
            ...
            _ip.validated_save()
            adapter.ipaddr_map[_ip.address] = _ip.id
            return super().create(ids=ids, adapter=adapter, attrs=attrs)
    ...
    def update(self, attrs):
        ...
        if attrs.get("status"):
            try:
                ...
                status = self.adapter.status_map[PLUGIN_CFG.get("default_status", "Active")]
