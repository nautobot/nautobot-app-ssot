"""Django views for data synchronization."""
...
class SyncListView(ObjectListView):
    ...
    table = SyncTable
    action_buttons = []
    template_name = "nautobot_data_sync/sync_home.html"
        ...
        if form.is_valid():
            ...
            transaction.on_commit(
                lambda: get_queue("default").enqueue(
                    "nautobot_data_sync.sync.sync", sync_id=sync.pk, data=form.cleaned_data, job_timeout=3600
                )
            )

            return redirect("plugins:nautobot_data_sync:sync", pk=sync.pk)
...
class SyncView(ObjectView):
    ...
    queryset = Sync.objects.all()
    template_name = "nautobot_data_sync/sync_detail.html"
