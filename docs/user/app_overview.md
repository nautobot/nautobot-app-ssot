...
* **Parallel adapter loading** loads source and target concurrently — enabled by default since SSoT 4.0.0; can be disabled per Job.
* **Bulk write helpers** are used by several shipped integrations (DNA Center, Meraki, ServiceNow) to replay the diff via `bulk_create` for faster writes; a generalized `BulkOperationsMixin` framework is *proposed* in [PR #1194](https://github.com/nautobot/nautobot-app-ssot/pull/1194) — see the [Performance chapter](performance.md#bulk-write-paths) for today's pattern.
* **Streaming pipeline** is *proposed* in [PR #1194](https://github.com/nautobot/nautobot-app-ssot/pull/1194): dump both adapters into a temporary SQLite store and run the diff via SQL set operations, capping memory usage and sidestepping the JSONField size limit on the persisted diff.
...
* **Per-integration enable flags** (`enable_<name>` keys in the app's `PLUGINS_CONFIG`, typically driven by environment variables) make each installed integration opt-in.

### Safety and correctness

* **Transaction control** — Nautobot 2.x runs Jobs without a default atomic wrapper; integrations can wrap writes in `transaction.atomic()` explicitly when all-or-nothing semantics are required. See the [Performance chapter](performance.md#transaction-boundaries).
* **Data validation strategies** *(layered, mostly DIY today)* — Pydantic source-shape validators, Django `clean_fields()` at dump time, and custom cross-row checks. A formalized phased validator registry is *proposed* in PR #1194; see [Performance & Validation Design Notes](../dev/performance_validation_menu.md) for the design.
* **Change logging** ranges from per-object signals (full audit) to deferred-batched (via Nautobot core's `deferred_change_logging_for_bulk_operation`) to none, depending on the chosen write path.
...
* **`BulkOperationsMixin`** *(proposed in [PR #1194](https://github.com/nautobot/nautobot-app-ssot/pull/1194))* — drop-in support for the proposed bulk-write framework on any integration's NautobotAdapter. Today, integrations using bulk writes implement them per-adapter (see DNA Center / Meraki / ServiceNow for examples).
...
* **Webhooks** fire on `ObjectChange` rows the sync produces (full per-object on the default `validated_save` path; none when integrations bulk-write outside a change context).
